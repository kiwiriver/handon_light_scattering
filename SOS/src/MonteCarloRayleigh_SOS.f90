...
!> modified 05/08/2014 to study the order of scatterings. 
...
Program MC
  ...
  Integer :: NphotonPrint, MaxScatt
  Logical :: l_Scatt

  !the number of photon used to print trajectory
  NphotonPrint=10

  !whether limit the order of scatterings
  l_Scatt=.False. ! calculate all the orders
  !  l_Scatt=.True.
  !the maximum order of scattering 
  MaxScatt=1
...
!  tau_Max=10.0_dp
  ...
  New_Photon : DO Kphoton=1,Nphoton  ! generate new photon
     ...
     Next_Scattering : DO WHILE(Weight>Weight_CutOff)
        !-- determine whether maximum number of scattering is reached --
        If (l_Scatt) Then
           If(ncol>=MaxScatt) CYCLE New_Photon
        End If
        ...
        !----SCATTERING----------------------------
        !--sample scattering angles for Rayleigh Scattering---
100     RN1=RAN2(IX)
        RN2=RAN2(IX)
  ...
  !get radiance---------
  Do i=1, Ntheta
     Radiance(i)=F0*Abs(Cos(theta0))*Radiance(i)/Nphoton
  ...
  If(l_Scatt) Then
     Print *, "number of orders traced:", MaxScatt
  Else
     Print *, "Reflection", Ref, "Transmission", Tran, "Loss", 1-(Ref+Tran)
     ! the reflection and transmission are only accurated for all orders of scattering
  End If ! 
